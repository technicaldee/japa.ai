    ...
    <div className="bg-background min-h-screen flex items-center justify-center font-body-md text-on-background p-margin-mobile md:p-margin-desktop" style={{backgroundImage:'radial-gradient(#d1e4ff 1px, transparent 1px)', backgroundSize:'20px 20px'}}>
      <main className="w-full max-w-[480px]">
        <div className="bg-surface-container-lowest rounded-xl card-shadow-lg p-md md:p-lg border border-outline-variant/30 flex flex-col gap-lg">
          ...
          {error && <div className="bg-error-container text-on-error-container p-sm rounded-lg font-body-md text-body-md text-center">{error}</div>}

          <div className="flex flex-col gap-sm">
            {[{icon:'G', label:'Sign in with Google'},{icon:'in', label:'Sign in with LinkedIn'}].map(({icon,label}) => (
              <button key={label} className="w-full flex items-center justify-center gap-base py-sm px-md rounded bg-surface-container border border-outline-variant hover:bg-surface-container-high transition-colors font-label-md text-label-md text-on-surface opacity-50 cursor-not-allowed" disabled>
          ...
          <form className="flex flex-col gap-md" onSubmit={handleSubmit}>
            <div className="flex flex-col gap-xs">
              ...
              <div className="relative">
                ...
                <input className="w-full pl-lg pr-sm py-sm rounded border border-outline-variant bg-surface-container-lowest text-on-surface focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all font-body-md text-body-md placeholder:text-on-surface-variant/50" id="email" placeholder="name@example.com" type="email" value={email} onChange={e=>setEmail(e.target.value)} required/>
              </div>
            </div>
            <div className="flex flex-col gap-xs">
              ...
              <div className="relative">
                ...
                <input className="w-full pl-lg pr-sm py-sm rounded border border-outline-variant bg-surface-container-lowest text-on-surface focus:border-secondary focus:ring-1 focus:ring-secondary outline-none transition-all font-body-md text-body-md placeholder:text-on-surface-variant/50" id="password" placeholder="••••••••" type="password" value={password} onChange={e=>setPassword(e.target.value)} required/>
              </div>
            </div>
            <button className="w-full py-sm px-md rounded bg-primary-container text-on-primary font-label-md text-label-md hover:bg-inverse-surface transition-colors mt-sm card-shadow flex items-center justify-center gap-xs" type="submit" disabled={loading}>
