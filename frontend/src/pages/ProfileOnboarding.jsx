...
export default function ProfileOnboarding() {
  ...
  const [form, setForm] = useState({
    highestDegree: '', fieldOfStudy: '', gpa: '',
    yearsOfExperience: '', jobTitle: '', budget: '', timeline: '',
  })
  ...
  useEffect(() => {
    profileApi.get()
      .then(p => {
        ...
        if (p.targetCountries) setSelected(p.targetCountries)
      })
      .catch(() => {})
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      await profileApi.update({ ...form, targetCountries: selected })
  ...
  if (loading) return <div className="bg-background text-on-background min-h-screen font-body-md flex"><Sidebar/><main className="flex-1 md:ml-64 pt-16 md:pt-0 p-md"><p>Loading profile...</p></main></div>
            ...
            <div className="col-span-1 md:col-span-9 flex flex-col gap-md">
              <div className="bg-surface-container-lowest rounded-xl p-md md:p-xl card-shadow border border-surface-variant">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-gutter mb-md">
                  <div className="flex flex-col gap-xs">
                    ...
                    <div className="relative">
                      <select className="w-full px-sm py-sm border border-outline-variant rounded-lg bg-surface-container-lowest text-body-md text-on-surface focus:outline-none focus:border-secondary appearance-none cursor-pointer" value={form.highestDegree} onChange={e=>setForm(f=>({...f,highestDegree:e.target.value}))}>
                  ...
                  <div className="flex flex-col gap-xs">
                    ...
                    <input className="w-full px-sm py-sm border border-outline-variant rounded-lg bg-surface-container-lowest text-body-md text-on-surface focus:outline-none focus:border-secondary" placeholder="e.g. Computer Science, Business" type="text" value={form.fieldOfStudy} onChange={e=>setForm(f=>({...f,fieldOfStudy:e.target.value}))}/>
                  </div>
                </div>
                <div className="flex flex-col gap-xs md:w-1/2">
                  ...
                  <div className="relative">
                    <input className="w-full pl-sm pr-10 py-sm border border-outline-variant rounded-lg bg-surface-container-lowest text-body-md text-on-surface focus:outline-none focus:border-secondary" placeholder="e.g. 3.8/4.0 or Second Class Upper" type="text" value={form.gpa} onChange={e=>setForm(f=>({...f,gpa:e.target.value}))}/>
              ...
              <div className="bg-surface-container-lowest rounded-xl p-md md:p-xl card-shadow border border-surface-variant">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-gutter">
                  <div className="flex flex-col gap-xs">
                    ...
                    <div className="relative">
                      <select className="w-full px-sm py-sm border border-outline-variant rounded-lg bg-surface-container-lowest text-body-md text-on-surface focus:outline-none focus:border-secondary appearance-none cursor-pointer" value={form.yearsOfExperience} onChange={e=>setForm(f=>({...f,yearsOfExperience:e.target.value}))}>
                  ...
                  <div className="flex flex-col gap-xs">
                    ...
                    <div className="relative">
                      <input className="w-full pl-sm pr-10 py-sm border border-outline-variant rounded-lg bg-surface-container-lowest text-body-md text-on-surface focus:outline-none focus:border-secondary" placeholder="e.g. Software Engineer" type="text" value={form.jobTitle} onChange={e=>setForm(f=>({...f,jobTitle:e.target.value}))}/>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-gutter">
                    {[{id:'budget',label:'Available Budget (USD)',opts:['Under $10,000','$10,000 - $30,000','$30,000 - $50,000','Over $50,000']},
                      ...
                      <div key={id} className="flex flex-col gap-xs">
                        ...
                        <div className="relative">
                          <select className="w-full px-sm py-sm border border-outline-variant rounded-lg bg-surface-container-lowest text-body-md text-on-surface focus:outline-none focus:border-secondary appearance-none cursor-pointer" value={form[id]} onChange={e=>setForm(f=>({...f,[id]:e.target.value}))}>
              ...
              <div className="flex flex-col-reverse md:flex-row justify-end items-center gap-sm mt-sm pt-md border-t border-outline-variant">
                <button className="w-full md:w-auto px-lg py-sm rounded-lg font-label-md text-label-md text-primary-container bg-transparent hover:bg-surface-variant transition-colors" onClick={handleSave} disabled={saving}>
                  {saving ? 'Saving...' : 'Save as Draft'}
                </button>
                <button onClick={handleContinue} disabled={saving} className="w-full md:w-auto px-lg py-sm rounded-lg font-label-md text-label-md text-on-primary bg-primary-container hover:bg-primary transition-colors card-shadow flex items-center justify-center gap-xs group">
