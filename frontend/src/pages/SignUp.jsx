...
export default function SignUp() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!terms) { setError('You must agree to the Terms and Conditions'); return }
    setError('')
    setLoading(true)
    try {
    ...
    <div className="bg-background min-h-screen flex text-on-background">
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-margin-mobile lg:p-margin-desktop bg-surface-container-lowest relative">
        ...
        <div className="w-full max-w-md space-y-lg mt-12 lg:mt-0">
          ...
          {error && <div className="bg-error-container text-on-error-container p-sm rounded-lg font-body-md text-body-md">{error}</div>}

          <form className="space-y-md" onSubmit={handleSubmit}>
            {[
              {id:'fullName', label:'Full Name', type:'text', placeholder:'Dr. Jane Doe', val:fullName, set:setFullName},
              {id:'email', label:'Email address', type:'email', placeholder:'jane.doe@example.com', val:email, set:setEmail},
            ...
            <div className="flex flex-col gap-xs">
              ...
              <div className="relative">
                <input className="w-full px-sm py-sm border border-outline-variant rounded-lg focus:outline-none focus:border-secondary focus:ring-2 focus:ring-secondary-container bg-surface-container-lowest text-on-surface font-body-md text-body-md transition-all card-shadow pr-10" id="password" placeholder="••••••••" type={showPass?'text':'password'} value={password} onChange={e=>setPassword(e.target.value)} required minLength={8}/>
            ...
            <div className="flex items-start gap-sm pt-xs">
              <input className="w-4 h-4 rounded border-outline-variant cursor-pointer mt-1" id="terms" type="checkbox" checked={terms} onChange={e=>setTerms(e.target.checked)}/>
            ...
            <button className="w-full flex justify-center py-sm px-md rounded-lg card-shadow font-label-md text-label-md text-on-primary bg-primary-container hover:bg-inverse-surface focus:outline-none transition-all duration-200" type="submit" disabled={loading}>
